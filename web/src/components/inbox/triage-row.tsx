...
function agoLabel(age: number | null): string | null {
  ...
  return `há ${Math.floor(age / 30)} meses`;
}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-faint">
          ...
          {!evaluated && <span className="italic text-muted">não avaliada</span>}
        </p>
      </div>
      ...
      {evaluated ? (
        <Link href={`/jobs/${scored!.jobId}`} className="flex shrink-0 items-center gap-1.5 text-xs">
          {scored!.running ? (
            <>
              ...
              <span className="text-brand max-sm:hidden">Avaliando…</span>
            </>
          ) : (
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            type="button"
            onClick={onSave}
            title={shortlisted ? "Na sua lista de interesse" : "Salvar na lista de interesse"}
          ...
          >
            ...
            <span className="max-sm:hidden">{shortlisted ? "Salva" : "Salvar"}</span>
          </button>
          <button
            type="button"
            onClick={onSkip}
            title="Ignorar — ocultar da caixa de entrada"
