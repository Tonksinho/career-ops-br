...
export function InboxTriage({ inbox }: { inbox: InboxJob[] }) {
  ...
  const skip = (job: InboxJob) => {
    ...
    setUndo({ label: `${job.company} ignorada`, fn: () => setHidden((h) => h.filter((u) => u !== job.url)) });
  };
    ...
    <div className={cn("mx-auto mt-4 max-w-3xl", shortlist.length > 0 && "pb-28 sm:pb-24")}>
      ...
      <div className="mt-4 flex items-baseline justify-between gap-3">
        <p className="text-sm font-medium text-foreground">
          {capped ? "Recentes — vale conferir" : anyFacet ? `${filtered.length} resultado${filtered.length === 1 ? "" : "s"}` : "Todas as vagas"}
        </p>
        {hiddenCount > 0 && (
          <button type="button" onClick={() => setHidden([])} className="text-xs text-faint transition-colors hover:text-foreground">
            {hiddenCount} ocultas · restaurar
          </button>
        )}
      </div>
      ...
      {selected.size > 0 && (
        <div className="mt-2 flex items-center gap-3 rounded-lg border border-brand/30 bg-brand-soft px-3 py-2 text-sm">
          <span className="font-medium text-brand tabular-nums">{selected.size} selecionadas</span>
      ...
      {visible.length > 0 ? (
        ...
        <div className="mt-3 rounded-2xl border border-dashed border-border bg-surface/30 px-6 py-10 text-center">
          ...
          <p className="mx-auto mt-1 max-w-sm text-sm text-muted">Amplie os filtros para ver mais vagas da caixa de entrada.</p>
        </div>
      )}
      ...
      {capped && ordered.length > BATCH && (
        ...
        >
          Ver todas as {ordered.length} vagas da caixa de entrada →
        </button>
      )}

      {/* empty-shortlist guidance (only once there's nothing saved) */}
      {shortlist.length === 0 && (
        <p className="mt-4 text-center text-xs text-faint">Salve as vagas interessantes e avalie todas juntas — um único lote de consumo.</p>
      )}
