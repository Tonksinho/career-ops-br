...
export function GeneratePdfButton({ n, company, pdfReady }: { n: string; company: string; pdfReady: boolean }) {
  ...
  const generate = () =>
    startJob({ title: `Currículo PDF · ${company}`, subtitle: "personalizado para esta vaga", kind: "pdf", input: n, page: `/pipeline/${n}` });
  ...
  if (job?.status === "running")
    return (
      <Link href={`/jobs/${job.id}`} className="inline-flex items-center justify-center gap-1.5 rounded-full border border-brand/40 bg-brand-soft px-3 py-1 text-xs font-medium text-brand max-sm:min-h-[44px]">
        <Loader2 className="size-3.5 animate-spin" /> Gerando currículo…
      </Link>
    );
  ...
  if (ready)
    return (
      <span className="inline-flex items-center gap-1">
        ...
        >
          <FileText className="size-3.5" /> Ver currículo personalizado
        </a>
        <button
          onClick={generate}
          title="Gerar novamente o currículo personalizado"
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      <button
        ...
        title="Gerar currículo otimizado para ATS e personalizado para esta vaga"
      >
        <FileDown className="size-3.5" /> Gerar currículo personalizado (PDF)
      </button>
