...
export function CvIngest({ onSaved }: { onSaved?: () => void }) {
  ...
  const runStream = useCallback(async (init: RequestInit) => {
    setPhase("parsing");
    setTrace("Lendo seu currículo…");
    setErr("");
    try {
      const r = await fetch("/api/cv/ingest", init);
      if (r.status === 404) {
        setErr("Conecte uma CLI de IA em Configurações para analisar seu currículo localmente.");
        setPhase("error");
        return;
      }
      if (!r.body) {
        setErr("Nenhuma resposta recebida.");
        setPhase("error");
        return;
      }
      ...
      for (;;) {
        ...
        if (parsed.error) {
          setErr(parsed.error === "unreadable" ? "Não consegui ler o texto desse arquivo; ele pode ser uma imagem digitalizada. Cole o texto do currículo." : "Não consegui analisar o currículo. Cole o texto diretamente.");
          setPhase("error");
          return;
        }
        if (parsed.trace) setTrace(parsed.trace.split("\n").filter(Boolean).slice(-1)[0] || "Lendo seu currículo…");
      ...
      if (!final.markdown.trim()) {
        setErr("Não encontrei um currículo legível. Cole o texto diretamente.");
        setPhase("error");
        return;
      }
      ...
      setErr(e instanceof Error ? e.message : "Erro durante a leitura.");
      setPhase("error");
    }
  }, []);
  ...
  const ingestFile = (file: File) => {
    ...
    if (/\.(md|markdown|txt)$/i.test(file.name)) {
      file
        ...
        .then((t) => {
          if (!t.trim()) {
            setErr("Esse arquivo parece vazio. Cole seu currículo diretamente.");
            setPhase("error");
            return;
          }
          setMd(t.trim());
        ...
        .catch(() => {
          setErr("Não consegui ler esse arquivo. Cole seu currículo diretamente.");
          setPhase("error");
        });
      return;
    }
  ...
  const save = async () => {
    if (!md.trim()) {
      setSaveErr("Seu currículo parece vazio. Cole o conteúdo novamente.");
      return;
    }
    setSaveErr("");
    setPhase("saving");
    try {
      ...
      if (!r.ok) {
        ...
        setSaveErr(d.error || "Não foi possível salvar seu currículo. Tente novamente.");
        setPhase("review"); // keep the parsed CV so they don't lose it
        return;
      }
    } catch {
      setSaveErr("Não foi possível salvar seu currículo. Verifique a conexão e tente novamente.");
      setPhase("review");
      return;
    }
    onSaved?.();
      ...
      <div className="space-y-3">
        ...
        >
          <textarea
            ...
            placeholder="Cole seu currículo aqui ou arraste um PDF/arquivo .md. Pode ser um texto sem formatação; nós organizamos para você."
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-border pt-3">
            ...
            >
              <Upload className="size-3.5" /> Enviar PDF ou arquivo
            </button>
            ...
            <span className="inline-flex items-center gap-1 text-[11px] text-faint">
              <Lock className="size-3" /> Fica no seu computador e é analisado pela sua própria IA.
            </span>
            <button
              type="button"
              disabled={!paste.trim()}
            ...
            >
              Ler meu currículo <ArrowRight className="size-4" />
            </button>
          </div>
        </div>
        {phase === "error" &&
          (err === "needs-cli" ? (
            <div className="flex flex-wrap items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2.5 text-[13px] text-amber-700 dark:text-amber-300">
              ...
              <span>Para ler um PDF, conecte sua CLI de IA ou cole o texto acima, sem configuração.</span>
              <Link href="/config" className="ml-auto inline-flex items-center gap-1 rounded-md bg-amber-500/20 px-2.5 py-1 font-medium text-amber-700 transition hover:bg-amber-500/30 dark:text-amber-200">
                Conectar minha CLI de IA <ArrowRight className="size-3.5" />
              </Link>
            </div>
          ) : (
    ...
    return (
      <div className="rounded-2xl border border-border bg-surface/60 p-6 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className={`${instrumentSerif.className} text-lg text-foreground`}>{trace || "Lendo seu currículo…"}</span>
        </div>
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">
          <span className="size-1.5 rounded-full bg-emerald-500" /> 0 tokens · R$ 0,00 · local
        </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface/60 p-4 backdrop-blur-sm md:p-5">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <h3 className={`${instrumentSerif.className} text-lg text-foreground`}>Seu currículo está pronto — revise e salve</h3>
        {readiness && (
          <span
            className={cn(
          ...
          >
            ...
            {readiness.scoreable ? "Pronto para buscar vagas" : "Faltam alguns detalhes"}
          </span>
        )}
      </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          Salvar e encontrar vagas compatíveis
        </button>
        <button
          type="button"
          onClick={() => {
        ...
        <span className="ml-auto inline-flex items-center gap-1 text-[11px] text-faint">
          <Lock className="size-3" /> Salvo localmente em cv.md
        </span>
      </div>
    </div>
  );
