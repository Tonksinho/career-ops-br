...
export function NextDateDialog({
  ...
  const call = async (method: "POST" | "DELETE", body: Record<string, unknown>, kind: "set" | "clear") => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(typeof j.error === "string" ? j.error : "Não foi possível atualizar a data fixada.");
        setBusy(null);
        return;
      }
      onChanged();
      ...
      setError("Não foi possível atualizar a data fixada.");
      setBusy(null);
    }
  };
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label={`Fixar próxima data de contato com ${entry.company}`}
        className="w-full max-w-sm rounded-2xl border border-border bg-surface p-5 shadow-xl"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="font-display text-lg">Fixar próximo contato</h2>
        ...
        >
          ...
          <p className="text-xs leading-relaxed text-faint">
            Substitui a agenda calculada até você registrar um contato; depois, a cadência normal é retomada.
          </p>
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            {entry.nextOverride && (
              <button
                ...
                title={`Atualmente fixado em ${entry.nextOverride}`}
              >
                {busy === "clear" ? <Loader2 className="size-3.5 animate-spin" /> : <PinOff className="size-3.5" />} Limpar data
              </button>
            )}
            ...
            >
              {busy === "set" ? <Loader2 className="size-3.5 animate-spin" /> : <Pin className="size-3.5" />} Fixar data
            </button>
          </div>
        </form>
      </div>
