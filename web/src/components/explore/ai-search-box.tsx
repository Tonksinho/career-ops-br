...
const EXAMPLES = [
  "Vagas de infraestrutura de IA em startups climáticas, remoto no Brasil",
  "Engenharia de soluções em startup Série A, remoto",
  "Liderança de IA aplicada em healthtech, publicada esta semana",
];
    ...
    <div>
      ...
      <div className="co-aibox p-4">
        <div className="mb-2 flex items-center gap-2 text-[12px] font-medium text-brand">
          <Sparkles className="size-3.5" /> Descreva a vaga — a IA fará a busca na web aberta
        </div>
        <textarea
          ref={ref}
          rows={2}
          ...
          placeholder="“Infraestrutura de IA em startups climáticas, remoto no Brasil” — escreva com suas palavras"
        />
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          <span className="text-[12px] text-muted">
            {cliConfigured ? (
              <>
                Pesquisa a web pública com <span className="text-foreground">{cliName || "sua CLI"}</span> — isso consome seus tokens.
              </>
            ) : (
              "Conecte uma CLI de IA nas Configurações para usar a busca com IA."
            )}
          </span>
          <button
            type="button"
