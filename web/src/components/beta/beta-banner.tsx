    ...
    <>
      <div className="fixed bottom-3 left-3 z-[70] flex items-center gap-2 rounded-full border border-brand/30 bg-surface/90 px-3 py-1.5 text-xs shadow-lg backdrop-blur-md">
        ...
        <button onClick={openReport} className="ml-1 inline-flex items-center justify-center gap-1 rounded-full bg-brand-soft px-2 py-0.5 font-medium text-brand-text transition-colors hover:bg-brand/15 max-sm:min-h-[44px]">
          <Bug className="size-3" /> Relatar problema
        </button>
      </div>

      {open && diag && (
        <div className="fixed inset-0 z-[96] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Relatar problema" onClick={() => setOpen(false)}>
          <div className="w-full max-w-lg rounded-2xl border border-border bg-[var(--bg)] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-foreground">Relatar problema · {diag.channel}</h2>
              <button onClick={() => setOpen(false)} aria-label="Fechar" className="ml-auto text-faint transition-colors hover:text-foreground">
                <X className="size-4" />
              </button>
            </div>
            <textarea
              ...
              rows={4}
              autoFocus
              placeholder="O que você estava fazendo e o que deu errado?"
            ...
            {desc.trim().split(/\s+/).length >= 3 && (
              ...
              >
                {searching ? <Loader2 className="size-3 animate-spin" /> : <Search className="size-3" />} Verificar relatos existentes
              </button>
            )}
            <details className="mt-3 rounded-lg border border-border bg-surface/40">
              <summary className="cursor-pointer select-none px-3 py-2 text-xs font-medium text-muted">Conteúdo exato que será anexado — revise antes de enviar ↓</summary>
            ...
            {similar.length > 0 && (
              <div className="mt-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2">
                <p className="text-xs font-medium text-amber-700 dark:text-amber-400">Já foi relatado? Reagir com 👍 em uma issue existente é melhor que duplicar:</p>
            ...
            <p className="mt-2 flex items-start gap-1.5 text-[11px] text-faint">
              <ShieldCheck className="mt-px size-3.5 shrink-0 text-emerald-500" /> Abre uma issue no GitHub para você confirmar. Nada é enviado antes do clique e nunca inclui currículo, perfil, respostas ou URLs de vagas.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                <Bug className="size-4" /> Abrir issue no GitHub
              </a>
            </div>
          </div>
        </div>
