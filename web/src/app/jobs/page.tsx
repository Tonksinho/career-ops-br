  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-display text-2xl tracking-tight text-landing">Tarefas</h1>
          <p className="mt-1 text-sm text-muted">
            Histórico permanente de todas as tarefas executadas. <span className="tabular-nums">{jobs.length}</span> no total.
          </p>
        </div>
        {jobs.some((j) => j.status !== "running") && (
          ...
          >
            <Trash2 className="size-3.5" /> Limpar concluídas
          </button>
        )}
      </div>
      ...
      {jobs.length === 0 ? (
        <div className="mt-8 rounded-2xl border border-dashed border-border bg-surface/30 px-6 py-12 text-center text-sm text-muted">
          Nenhuma tarefa ainda. Clique em <span className="text-foreground">Avaliar</span> em uma vaga da caixa de entrada para começar.
        </div>
      ) : (
            ...
            return (
              <li key={j.id}>
                <Link href={`/jobs/${j.id}`} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-surface-hover">
                  ...
                  <span className="hidden shrink-0 text-xs text-faint sm:block">{j.status === "running" ? "em execução" : j.status === "done" ? "concluída" : "erro"}</span>
                </Link>
              </li>
            );
          })}
