...
const ACTIONS: Record<string, ActionDef> = {
  ...
  evaluate: {
    ...
    run: (raw, ctx) => {
      const url = raw.url;
      if (!isStr(url) || !/^https?:\/\//i.test(url)) return { status: "ignored", note: "URL inválida." };
      const ex = ctx.jobForUrl(url);
      if (ex && ex.status !== "error" && !raw.rerun) return { status: "ignored", note: "Esta vaga já foi avaliada." };
  ...
  evaluateCompany: {
    ...
    run: (raw, ctx) => {
      ...
      if (!isStr(company)) return { status: "ignored", note: "Empresa não informada." };
      ...
      if (pending.length === 0) {
        return {
          status: "ignored",
          note: matches.length > 0 ? `Todas as vagas da ${company} já foram avaliadas.` : `Não há vagas pendentes da ${company} na caixa de entrada.`,
        };
      }
      ...
      return {
        status: "confirm",
        summary: `Avaliar ${pending.length} vagas da ${company}? (~${pending.length} tarefa${pending.length > 1 ? "s" : ""})`,
        run: fire,
      };
    },
  },
  ...
  explore: {
    ...
    sideEffect: "none",
    run: (raw, ctx) => {
      if (!ctx.applyExplore) return { status: "ignored", note: "A busca de vagas não está disponível aqui." };
      const run = raw.run === true;
      const merge = raw.merge === true;
      ctx.push("/explore");
      ctx.applyExplore(raw, { merge, run });
      return { status: "done", note: run ? "Buscando vagas recentes na rede de ATS (grátis)…" : "Busca aberta com seus filtros." };
    },
  },
  ...
  research: {
    ...
    run: (raw, ctx) => {
      ...
      if (!isStr(target)) return { status: "ignored", note: "Alvo da pesquisa não informado." };
      const id = ctx.startJob({
        title: isStr(raw.title) ? String(raw.title) : "Pesquisar",
        kind: "research",
        input: target,
        page: "/pipeline",
      });
  ...
  generatePdf: {
    ...
    run: (raw, ctx) => {
      ...
      if (!n) return { status: "ignored", note: "Informe o número da candidatura." };
      const app = ctx.applications.find((a) => a.n === n);
      const id = ctx.startJob({ title: `Currículo PDF · ${app?.company ?? `#${n}`}`, subtitle: "currículo personalizado", kind: "pdf", input: n, page: `/pipeline/${n}` });
      return { status: "done", jobIds: id ? [id] : [] };
    },
  },
  ...
  setStatus: {
    ...
    run: (raw, ctx) => {
      ...
      if (!n || !canon) return { status: "ignored", note: "Informe o número da candidatura e um status válido." };
      const app = ctx.applications.find((a) => a.n === n);
      const label = app ? `${app.company} · ${app.role}` : `#${n}`;
      return {
        status: "confirm",
        summary: `Marcar ${label} como ${statusLabel(canon)}?`,
        run: () => {
          ctx.writeStatus(n, canon);
          return { note: `Candidatura #${n} marcada como ${statusLabel(canon)}.` };
        },
      };
    },
  },
  ...
  apply: {
    ...
    run: (raw, ctx) => {
      const url = raw.url;
      if (!isStr(url) || !/^https?:\/\//i.test(url)) return { status: "ignored", note: "Informe a URL do formulário de candidatura." };
      ctx.startApply(url);
      return { status: "done", note: "Abrindo o formulário de candidatura…" };
    },
  },
  ...
  setApplyField: {
    ...
    run: (raw, ctx) => {
      ...
      if (!isStr(field) || typeof value !== "string") return { status: "ignored", note: "Informe o campo e o valor." };
      ctx.setApplyField(String(field), value);
      return { status: "done", note: `Campo “${field}” atualizado.` };
    },
  },
  ...
  setProfile: {
    sideEffect: "write",
    run: (raw, ctx) => {
      if (!ctx.writeProfile) return { status: "ignored", note: "Não é possível salvar o perfil aqui." };
      const p = coerceProfile(raw);
      const has = Object.values(p).some((v) => (Array.isArray(v) ? v.length : v !== undefined));
      if (!has) return { status: "ignored", note: "Nada para salvar." };
      const bits = [p.roles?.length ? `cargos: ${p.roles.join(", ")}` : "", p.location ? `em ${p.location}` : "", p.compMin && p.compMax ? `remuneração ${p.compMin}–${p.compMax}` : ""].filter(Boolean).join(" · ");
      return {
        status: "confirm",
        summary: `Salvar seu perfil?${bits ? ` (${bits})` : ""}`,
        run: () => {
          ctx.writeProfile!(p as Record<string, unknown>);
          if (p.roles?.length) ctx.writePortals?.(p.roles, p.location ? [p.location] : undefined);
          return { note: "Perfil salvo — as correspondências ficarão mais precisas." };
        },
      };
    },
  },
